...
- ✅ **Runtime verified in a real Chrome (2026-09-06):** `npm run e2e` (puppeteer-core over the DevTools pipe) installs the production build and passes 10/10 checks — PGlite under the CSP, offline ONNX embeddings, capture/dedup/retrieve, a cited grounded answer + calibrated refusal from `qwen2.5:3b-instruct` via Ollama, and the CDP a11y snapshot. `npm run fetch-model` now also bundles the ONNX WASM engine (it was being fetched from a CDN).
- 📐 **Plan revised 2026-09-06** — see [`docs/07`](docs/07_REVISED_PLAN_2026-09.md): the first paper is a benchmark + calibration study + refusal-aware fine-tuning paper (QA track only); the action-grounding track is deferred; benchmark **v0 is a harness validator, not a claims-grade dataset** (it is template-separable) and is replaced by a real-source v1 with Wikipedia-revision stale/fresh pairs. Caveat on the status lines above: "calibrated" currently means a fixed retrieval-score cutoff plus a prompt instruction; no learned calibration exists yet.
