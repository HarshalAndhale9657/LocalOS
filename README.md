...
- 👁 **M1.2 observe + 🖱 M1.3 act implemented** (build + typecheck): CDP a11y snapshot → compressed indexed nodes + `versionId`; executor maps `index → backendNodeId → CDP` (click/type/scroll/navigate) with a stale-snapshot guard + SAFE/CAUTION/UNSAFE confirmation gate. The observe→act substrate is in place. *Runtime pending a Chrome load.*
- ⏭ Next: verify in Chrome → M1.5 grounded answering + the agent loop that *decides* actions (base model via Ollama → first Personal-Memory-RGB number) + confirmation-card UI/action log. See [`docs/06`](docs/06_M1_BUILD_PLAN.md).
