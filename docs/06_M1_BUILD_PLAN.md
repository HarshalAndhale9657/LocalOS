...
- **M1.1 — done.** Scaffold builds + typechecks; side panel, offscreen host, background router, `lib/` skeleton.
- **M1.2 — implemented (builds + typechecks); runtime pending a Chrome load.** CDP wrapper (`lib/observe/cdp.ts`, attach keeps the SW alive), accessibility-tree compression to on-task interactive/context nodes with stable element indices + a FNV `versionId` (`axtree.ts`), executor-only `backendNodeId` stripped before the model sees it (`forModel`), and an `OBSERVE_ACTIVE_TAB` route returning a compact summary for verification. **Next for the action track:** M1.3 executor maps `index → backendNodeId → CDP Input.dispatch*` with the version-id guard + confirmation gates.
- **M1.3 — implemented (builds + typechecks); runtime pending a Chrome load.** Executor (`lib/act/index.ts`) maps element `index → backendNodeId → CDP` (`Input.dispatchMouseEvent` click, `Input.insertText` type, `Runtime.evaluate` scroll, `chrome.tabs`/`history` navigate). Two guards enforced: **stale-snapshot** (`action.versionId` must match the observation) and **confirmation** (non-SAFE actions return `needsConfirm`). Risk classifier flags submit/pay/delete-like clicks UNSAFE. *Still to wire:* the agent loop that *decides* actions (M1.5) + the confirmation-card UI + action log.
- **M1.5 (QA half) — implemented (builds + typechecks); runtime needs Ollama.** `lib/model/index.ts` = Ollama client with a grounded-QA prompt (numbered SOURCES, inline `[n]` citations, calibrated `"Not found in your history."` refusal, instruction-hierarchy note not to obey source text), page sources wrapped by `spotlight()` and `guardRetrieved()` (untrusted). Background `ASK` route retrieves → answers, degrading to retrieval-only (no hallucination) when Ollama is unreachable; the side-panel "Ask" renders the cited answer/refusal. **To run:** install Ollama, `ollama pull qwen2.5:7b-instruct`, allow the extension origin via `OLLAMA_ORIGINS`.
- **Agentic task loop — implemented (builds + typechecks); runtime needs Ollama.** `lib/model.act()` decides the next action (JSON: verb/index/args/reason) from a spotlighted observation; `lib/agent` runs the loop **observe → decide → risk-gate → confirm → execute → re-observe** (max steps, cancelable), streaming `AGENT_EVENT`s to the panel. The side panel gained an **Ask/Do mode toggle**, a live **action log**, a **confirmation card** for non-SAFE actions (Approve/Reject), and a **Stop** button. Background routes: `RUN_TASK` / `CONFIRM_RESULT` / `CANCEL_TASK`. *Still to do:* M1.6 (extension vs Personal-Memory-RGB base number), the `extract` verb's structured output, and the settings/privacy panel.
- **M1.4 — implemented (builds + typechecks); runtime verification pending a Chrome load.** Done: Readability capture (content script), chunking, SimHash dedup, transformers.js embeddings (`bge-small-en-v1.5`, offscreen), PGlite persistence, retrieve = cosine + exponential time-decay + MMR + negative-rejection threshold, SW↔offscreen plumbing, and a wired side panel ("Remember page" + "Ask" → grounded chunks). **Deviations/caveats to close:** (1) v1 does JS-side cosine (pgvector `./vector` subpath isn't in pglite 0.5.6 — swap to pgvector+HNSW behind `searchByVector` later); (2) ~~transformers.js fetches the embedding model from the HF Hub~~ **DONE** — model bundled offline via `npm run fetch-model` (weights gitignored) + transformers.js configured for local-only load; still confirm the ORT-wasm *runtime* loads from the bundled asset offline; (3) PGlite emits a direct-`eval` that must be checked against the extension CSP (`wasm-unsafe-eval`) at runtime; (4) add the hybrid **BM25 + cross-encoder reranker** (only dense retrieval so far).
