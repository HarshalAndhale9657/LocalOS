...
- **M1.4 — implemented (builds + typechecks); runtime verification pending a Chrome load.** Done: Readability capture (content script), chunking, SimHash dedup, transformers.js embeddings (`bge-small-en-v1.5`, offscreen), PGlite persistence, retrieve = cosine + exponential time-decay + MMR + negative-rejection threshold, SW↔offscreen plumbing, and a wired side panel ("Remember page" + "Ask" → grounded chunks). **Deviations/caveats to close:** (1) v1 does JS-side cosine (pgvector `./vector` subpath isn't in pglite 0.5.6 — swap to pgvector+HNSW behind `searchByVector` later); (2) transformers.js fetches the embedding model from the HF Hub on first use — **bundle/host the ONNX model** for the true-offline/privacy guarantee; (3) PGlite emits a direct-`eval` that must be checked against the extension CSP (`wasm-unsafe-eval`) at runtime; (4) add the hybrid **BM25 + cross-encoder reranker** (only dense retrieval so far).
