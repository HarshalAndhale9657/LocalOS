...
- **M1.5 (QA half) — implemented (builds + typechecks); runtime needs Ollama.** `lib/model/index.ts` = Ollama client with a grounded-QA prompt (numbered SOURCES, inline `[n]` citations, calibrated `"Not found in your history."` refusal, instruction-hierarchy note not to obey source text), page sources wrapped by `spotlight()` and `guardRetrieved()` (untrusted). Background `ASK` route retrieves → answers, degrading to retrieval-only (no hallucination) when Ollama is unreachable; the side-panel "Ask" renders the cited answer/refusal. **To run:** install Ollama, `ollama pull qwen2.5:7b-instruct`, allow the extension origin via `OLLAMA_ORIGINS`. *Still to do for the action track:* the deciding agent loop (`model.act` + observe + execute) + confirmation-card UI + action log; and M1.6 wiring the extension against Personal-Memory-RGB for the base-model number.
