...
### 2026-09-06 (week 1, day 1 — v1 generation pipeline)
- **Teacher client** `pmrgb/teacher.py`: provider-agnostic (OpenAI wire format — Ollama, Groq, Gemini, OpenAI, Anthropic all work by env var), every call disk-cached by content hash, and a token **ledger** at `data/cache/teacher_ledger.jsonl` so the 5–10 USD budget is auditable. Default is the local 3B, so the pipeline runs at zero cost; a better teacher is a two-env-var swap and re-runs are free for unchanged prompts.
- **Article pool** `pmrgb/corpus.py`: snapshots each Wikipedia article at fixed **read dates** (2024-02-01, 2025-03-01) and at **NOW** (2026-09-01), keyed `title@read_date`, and labels every numeric sentence in the read text as `stale` / `fresh` / `stable`. 8 personas x ~25 titles in `sources/personas_v1.json`.
- **Two labeling rules found necessary by inspecting real output** (both unit-tested):
  1. *Staleness vs source correction.* A numeric change is only stale if the sentence's **own time anchor moves forward** ("In 2023 ... 19.9% share" -> "In 2024 ... 17.6%"). Changes where the anchor stays put are Wikipedia **correcting a past error** ($100m -> $176m for a 2004 event) and changes where it moves backward are date fixes; both are excluded rather than silently mislabeled as stale.
  2. *Fresh controls must be current-state claims.* "In 1975, the polka-dot jersey was introduced" is a historical event that can never go stale, so an unchanged such sentence proves nothing. `fresh` now requires an "as of / since / currently" anchor, parallel to the stale class.
- **Generator** `pmrgb/generate_v1.py`: assembles persona histories (real pages, boilerplate, near-duplicate revisits, timestamps sampled inside the window where that revision was demonstrably live) and generates `single_hop`, `multi_hop`, `time_sensitive_fresh`, `time_scoped`, `stale`, `not_in_history`, `false_premise`. Splits **by history and by source article**, dropping items rather than letting an article's facts appear in two splits.
- **Three generator defects found by reading the output and fixed:** answers leaking into their own question ("What did the jersey start awarding in 1975?" -> "1975"); `time_scoped` questions with several correct answers (now restricted to days when exactly one page was read); and free-form multi-hop from a small teacher producing two unrelated questions stapled together (now **computed comparisons**: same unit, same persona topic, gold answer derived here, teacher only phrases it).
- **Known limitation, by design for now:** the lexical gate cannot catch teacher *comprehension* errors (e.g. "first rider since Eddy Merckx" turned into a question whose gold answer is Merckx). This is what the NLI gate and the human verification pass are for; a stronger teacher will also cut the rate. Gate currently rejects ~35% of candidates.
- **Stale yield is the open risk:** ~0.2 stale facts per snapshot across general articles (vs ~0.75 on high-churn ones). Extrapolating, the current title list gives ~90 stale items, short of the target. Mitigations, in order of cost: add a third read date, add high-churn titles (sports records, company statistics, city populations), and accept `changed_fuzzy` candidates that pass human review.
